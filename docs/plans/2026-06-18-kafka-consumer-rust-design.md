...
Date: 2026-06-18
Status: Milestone A implemented on `solution`; stubbed as the exercise on `master`. Milestones B–C planned.
...
- **Milestone A — in-memory (this doc):** in-memory fakes behind traits. No
  ...
  `Arc<Mutex<…>>` shared state, dependency inversion.
...
- **Milestone C — real Kafka:** swap the in-memory bus for an `rdkafka`
  consumer. Teaches real broker config, offsets, acks.

Each swap is a small diff that does **not** touch business logic — that is the
Rust dependency-inversion lesson. The only files every milestone touches are
`src/main.rs` (wiring), `src/state.rs` (shared-state type), and `Cargo.toml`
(new deps); those are the entire conflict surface.

## Branch model: exercise vs. solution

The repo runs two **parallel tracks** that diverge on purpose (same files,
stubbed bodies vs. real bodies), so they are never merged into each other:

- **`master` — the growing exercise.** A single linear exercise you work through
  A → B → C. Each milestone is added on top with its new bodies left as
  `todo!()`, so `cargo test` is red until you implement it.
- **`solution` — the reference stack.** The same project fully implemented,
  authored as a clean linear stack: each milestone branches off the current
  `solution` tip, gets implemented, and merges back fast-forward.

Authoring loop per milestone: implement fully on the `solution` stack, then bring
the milestone's **new pieces onto `master` with bodies stubbed**. Because the
work is additive behind the fixed traits, that is mostly new files plus the small
`main.rs`/`state.rs`/`Cargo.toml` edits noted above.
